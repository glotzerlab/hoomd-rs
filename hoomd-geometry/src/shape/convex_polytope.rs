...
impl<const MAX_VERTICES: usize> ConvexPolytope<4, MAX_VERTICES> {
    ...
    ///
    /// This shape's vertices are defined by the permutations of `{±1/√2, ±1/√2, 0, 0}`.
    ///
    /// # Example
    /// ```
    /// use approxim::assert_relative_eq;
    ...
    /// assert_relative_eq!(octaplex.bounding_sphere_radius().get(), 1.0);
    ///
    /// // All vertices are the same distance from the origin
    /// assert!(octaplex.vertices().iter().all(|v| (v.norm_squared() - 1.0).abs() < 1e-10));
    /// # Ok(())
    /// # }
    /// ```
    #[expect(clippy::missing_panics_doc, reason = "hard-coded constant is positive")]
    pub fn octaplex() -> Self {
        ...
        for (l, r) in [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)] {
            for ls in [-1.0, 1.0] {
                for rs in [-1.0, 1.0] {
                    vertices.push(
                        std::array::from_fn(|i| {
                            (f64::from(i == l) * ls + f64::from(i == r) * rs) * s
                        })
                        .into(),
                    );
                }
        ...
        Self {
            vertices,
            bounding_radius: 1.0_f64.try_into().expect("hard-coded constant is positive"),
        }
    }
}
