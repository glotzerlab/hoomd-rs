...
use hoomd_interaction::{pairwise::{Boxcar, Isotropic, IsotropicEnergy}, CutoffPair};

struct HardSphereWell {
    maximum_interaction_range: f64
}

pub struct Step<const D: usize, X> {
    ...
    hamiltonian: CutoffPair<Isotropic<HardSphereWell>>,
    macrostate: Isothermal,
}

impl IsotropicEnergy for HardSphereWell {
    fn energy(&self, r: f64) -> f64 {
        let well = Boxcar {
            epsilon: -0.2,
            left: 0.0,
            right: self.maximum_interaction_range,
        };
...
impl<const D: usize, X> Simulation for Step<D, X> where
...
impl<const D: usize> Step<D, VecCell<SiteKey, D>> where
Periodic<Hypercuboid<D>>: GenerateGhosts<Point<Cartesian<D>>>,
{
    pub fn with_microstate<X>(microstate: &Microstate<Point<Cartesian<D>>, Point<Cartesian<D>>, X, Periodic<Hypercuboid<D>>>) -> anyhow::Result<Self> {
        ...
        let hamiltonian = CutoffPair {
            ...
            evaluator: Isotropic(HardSphereWell { maximum_interaction_range, }),
        };    
