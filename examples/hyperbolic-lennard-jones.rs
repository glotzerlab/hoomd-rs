...
use hoomd_geometry::shape::TwelveTwelve; //EightEight;
...
const RHO: f64 = 1.0;
const PARTICLE_NUMBER: usize = 350;
const DIAMETER: f64 = 0.35; //in Hyperbolic metric
...
struct Fill {
    ...
    microstate: Microstate<
        ...
        Point<Hyperbolic<3>>,
        AllPairs<SiteKey>,
        Periodic<TwelveTwelve>, //EightEight>,
    >,
...
impl Fill {
    ...
    fn new() -> anyhow::Result<Fill> {
        let boundary = Periodic::new(TwelveTwelve::CUSP_TO_EDGE, TwelveTwelve {skirt: 1.0_f64})?; //EightEight::CUSP_TO_EDGE, EightEight { skirt: 1.0_f64 })?;
        let mut microstate =
            Microstate::builder().boundary(boundary).try_build()?;

        let initial_spacing = 2.5;
