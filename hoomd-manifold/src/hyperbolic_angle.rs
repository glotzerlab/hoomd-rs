...
impl Distribution<HyperbolicAngle> for StandardUniform {
    /** Sample a random hyperbolic angle from the uniform distribution over 
    a between 0 and 2pi, and b and c between -1 and 1

    # Example

    ```
    use hoomd_manifold::HyperbolicAngle;
    ...
    */
    ...
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> HyperbolicAngle {
        ...
        let uniform_boost = Uniform::new(-1.0, 1.0).expect("hard-coded distribution should be valid");
        HyperbolicAngle::from((uniform_angle.sample(rng), uniform_boost.sample(rng), uniform_boost.sample(rng)))
    }
}