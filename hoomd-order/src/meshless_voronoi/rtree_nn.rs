...
impl<'a> RTreeWrappingNearestNeighbourIter<'a, GeneratorHyperbolic> {
    pub fn new_hyperbolic(
        root: &'a ParentNode<GeneratorHyperbolic>,
        query_point: [f64; 3],
        width: [f64; 3],
        dimensionality: Dimensionality,
    ) -> Self {
...
impl WrappingPointDistance for GeneratorHyperbolic {
    fn wrapping_distance_2(&self, point: &[f64; 3], shift: &[f64; 3]) -> f64 {
        let dx = [
            point[0] + shift[0] - self.loc[0],
            point[1] + shift[1] - self.loc[1],
...
impl<'a> Iterator for RTreeWrappingNearestNeighbourIter<'a, GeneratorHyperbolic> {
    type Item = (&'a GeneratorHyperbolic, f64, [f64; 3]);
        ...
        while let Some(current) = self.nodes.pop() {
            match current {
                RTreeNodeDistanceWrapper {
                    ...
                    self.extend_heap_hyperbolic(data.children(), shift);
                }
                RTreeNodeDistanceWrapper {
                    node: RTreeNode::Leaf(t),
                    distance,
