...
struct HardSphere {
    microstate: Microstate<BodyProperties, SiteProperties, VecCell<SiteKey, 3>, Periodic<Hypercuboid<3>>>,
    translate_sweep: Sweep<Translate<PositionVector>>,
    hamiltonian: CutoffPairOverlap<AlwaysTrue>,
    macrostate: Isothermal,
}

impl Simulation for HardSphere {
...
fn main() -> anyhow::Result<()> {
    ...
    info!("Hard sphere benchmark: {} spheres at number density {}", n, number_density);
    ...
    let mut hard_sphere = HardSphere {
        microstate,
        translate_sweep,
        hamiltonian,
    ...
    benchmarks::benchmark(&mut hard_sphere, 1000, 1000, 30)?;
