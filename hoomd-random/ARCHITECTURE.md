...
reproducible results, consistent performance, and support for parallel execution.
The `hoomd_random` crate provides a unified API so that other parts of hoomd-rs can
...
Not all algorithms will use all counters, so `hoomd_random` implements the builder
pattern to construct the seed along these lines:
