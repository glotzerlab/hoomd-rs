...
impl<F> Xplor<F>{
  ...
  fn s(&self, r: f64) -> f64 {
    if r < self.r_on {
      ...
      (r_cut_sq - r_sq).powi(2) * (r_cut_sq + 2.0 * r_sq-3.0 * r_on_sq) / (r_cut_sq - r_on_sq).powi(3)
    }
  }
  /// The xplor shifting function
  #[inline]
  fn ds_dr(&self, r: f64) -> f64 { // TODO: can we share r_*sq between energy and force?
    if r < self.r_on {
      1.0 // TODO: is this still correct? I think not?
    } else if r > self.r_cut {
      0.0 
    } else {
      let r_sq = r.powi(2);
...
impl<F: IsotropicEnergy> IsotropicEnergy for Xplor<F> {
    ...
    // is seperate in this version, users can just not xplor on WCA
}

impl<F: IsotropicForce + IsotropicEnergy> IsotropicForce for Xplor<F> {
    #[inline]
    fn force(&self, r: f64) -> f64 {
        // Chain rule of s(r) * U(r). TODO: not equal to f at r < r_on - should it be?
...
mod tests {
    ...
    fn special_points_12_6(
        ...
        assert_abs_diff_ne!(xplor_lj.energy(r_on+1e-4), lj.energy(r_on+1e-4));
