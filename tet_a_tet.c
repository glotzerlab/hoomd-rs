 ...
 * tet_a_tet.h                                                           o o *
 *                                                                o     o *
 * Visual Computing Group                                         _  O  _ * IEI
 *Institute, CNUCE Institute, CNR Pisa                        \/)\/          *
 *                                                                /\/| *
...
static double P_V1[4][3],
    P_V2[4][3]; // differences between the vertices of the second (first)

//  tetrahedron

// and the vertex 0  of the first(second) tetrahedron

static double Coord_1[4][4],
    Coord_2[4][4]; // vertices coordinates in the affine space
...
inline static bool FaceA_1(double *Coord, int &maskEdges) {
  ...
  return (
      maskEdges ==
      017); // if true it means that all of the vertices are out the halfspace

  // defined by this face
}
// it is the same as FaceA_1, only the values V2[0]-v_ref are used only for the
// fourth face
...
inline static bool FaceB_2() {
  double *v_ref = V2[1];
  return ((SUB_DOT(V1[0], v_ref, n) > 0) && (SUB_DOT(V1[1], v_ref, n) > 0) &&
          (SUB_DOT(V1[2], v_ref, n) > 0) && (SUB_DOT(V1[3], v_ref, n) > 0));
}
