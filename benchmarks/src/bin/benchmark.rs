...
fn execute<'a, S>(results: &mut BTreeMap<&'a str, Performance>,
              simulation: &mut S,
              benchmark: &Benchmark,  
              name: &'a str,
...
fn main() -> anyhow::Result<()> {
    ...
    if n_max != n && !options.json {
        return Err(anyhow!("JSON output is required when n_min ({n}) is not equal to n_max ({n_max})"));
    }
    ...
    loop {
        ...
        if benchmark_matcher.matches(name) {
            let microstate_2d = &maybe_microstate_2d.as_ref().expect("microstate_2d should be initialized");
        ...
        if benchmark_matcher.matches(name) {
            let microstate_3d = &maybe_microstate_3d.as_ref().expect("microstate_3d should be initialized");
            match options.spatial_data {
                SpatialData::VecCell => {
                    let mut simulation =
    ...
    if options.json {
        ...
        for (name, performance) in results {
            let operations_per_second = 1.0 / performance.time_per_operation.last().expect("results should contain at least one measurement");
            println!("{name:20}: {operations_per_second:9.3} {}s/s", performance.unit);
        }
    }
