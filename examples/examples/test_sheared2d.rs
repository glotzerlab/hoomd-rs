...
impl SiteType {
    const VARIANTS: &'static [&'static str] = &["Rhomboid"];
}

const RHOMBUS_TYPE_SHAPE: &str = r#"{"type": "Polygon", "vertices": [[-0.75, -0.4330127018922193], [0.25, -0.4330127018922193], [0.75, 0.4330127018922193], [-0.25, 0.4330127018922193]]}"#;
...
struct HardRhomboidSelfAssembly {
    ...
    hamiltonian: PairwiseCutoff<HardShape<Rhomboid>>,
...
impl HardRhomboidSelfAssembly {
    ...
    fn new() -> anyhow::Result<HardRhomboidSelfAssembly> {
        let initial_packing_fraction = 0.05;
        let target_packing_fraction = 0.07;
        let n_bodies = 2;
        ...
        let hamiltonian = PairwiseCutoff(HardShape(rhomboid.clone()));
...
fn main() -> anyhow::Result<()> {
    let trajectory_path = Path::new("hard-rhomboid-self-assembly.gsd");
    gsd::create_with_type_shapes(
        trajectory_path,
        SiteType::VARIANTS,
        &[SQUARE_TYPE_SHAPE, RHOMBUS_TYPE_SHAPE],
    )?;
    Ok(())
}
