...
let manifest_file = fs::read_to_string("examples/Cargo.toml").context("error reading examples/Cargo.toml")?;
let cargo = manifest_file.parse::<Table>().context("error parsing examples/Cargo.toml")?;
let examples = cargo.get("package").ok_or(anyhow!("package key not present"))?.get("metadata")
...
fn build_examples(table: &Table, args: &Args) -> anyhow::Result<()> {
    let profile = if args.reduce_size { "release-wasm" } else { "release" };
    ...
    for (name, configuration) in table {
        println!("{name}");
        let configuration = configuration.as_table().ok_or(anyhow!("package.metadata.example.{name} is not a table"))?;
        let subpath = configuration.get("path").ok_or(anyhow!("package.metadata.example.{name}.path not found"))?
            .as_str().ok_or(anyhow!("package.metadata.example.{name}.path is not a string"))?;
        ...
        fs::rename(optimized_wasm, output_wasm.as_str()).context(format!("failed to move {optimized_wasm} to {output_wasm}"))?;
        }
        }
