...
use hoomd_vector::{Cartesian, Outer, Rotate, Versor, Wedge};
...
const NDIMS: usize = 3;
const DUMBBELL_END_POSITON: Cartesian<NDIMS> = Cartesian { coordinates: [0.0, 0.25, 0.0] };
const ZERO_MOI: [f64; 3] = [0.0, 0.0, 0.0];
        ...
        self.append_frame(microstate.step())?
            ...
            .particles_position(
                microstate
                    ...
                    .map(|s| s.properties.position) // update this for 2D
            )?
            .particles_type_id(
                microstate
                    .iter_sites_tag_order()
...
impl SystemParams {
    ...
    pub fn density(&self) -> f64 {
        match self.particle_type {
            BodyVariants::Sphere => crate::DENSITY_SPHERE_3D,       // update this for 2D
            BodyVariants::Dumbbell => crate::DENSITY_DUMBBELL_3D,   // update this for 2D
        }
    }
...
pub fn make_microstate(system: &SystemParams) -> anyhow::Result<Microstate> {
    ...
    let boundary = Periodic::new(
        crate::R_CUT,
        Cuboid::with_equal_edges(box_side_length.try_into()?),  // update this for 2D
    )?;
    ...
    for index in [(0..system.particles_per_side), (0..system.particles_per_side), (0..system.particles_per_side)]   // update this for 3D
        .into_iter()
