...
use hoomd_manifold::{Minkowski, HyperbolicTranslate, EightEight, Hyperboloid, FundamentalDomain};
...
fn run(mut terminal: DefaultTerminal) -> Result<(), Box<dyn std::error::Error>> {
    ...
    .bodies([Body::point(Minkowski::from([0.00001,0.00001,sqrt(2.0*(0.00001_f64).powi(2) + RHO.powi(2))]))])
    .try_build()?;
